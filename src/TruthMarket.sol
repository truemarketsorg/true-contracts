...
contract TruthMarket is Initializable, OwnableUpgradeable, OraclePausable, ReentrancyGuardUpgradeable {
    ...
    uint256 private paymentTokenDecimals;
    ...
    function initialize(
        ...
        tokenDecimals = yesToken.decimals();

        IUniswapV3Factory uniswapV3Factory = IUniswapV3Factory(marketManager.uniswapV3Factory());
        yesPool = uniswapV3Factory.createPool(_yesToken, _paymentToken, POOL_FEE);
        noPool = uniswapV3Factory.createPool(_noToken, _paymentToken, POOL_FEE);
    ...
    function mint(uint256 paymentTokenAmount) external notPaused nonReentrant {
        ...
        uint256 tokenAmount = paymentTokenAmount * (10 ** tokenDecimals) / (10 ** paymentTokenDecimals);

        // make sure token cap not exceeded
        if (yesToken.totalSupply() + tokenAmount > yesNoTokenCap) {
    ...
    function burn(uint256 amount) external notPaused nonReentrant {
        uint256 paymentTokenAmount = amount * (10 ** paymentTokenDecimals) / (10 ** tokenDecimals);
    ...
    function withdrawFromCanceledMarket() external notPaused nonReentrant {
        ...
        uint256 paymentTokenAmount = totalBalance * (5 * 10 ** (paymentTokenDecimals - 1)) / (10 ** tokenDecimals);
    ...
    function getUserClaimableAmount(address _account) public view returns (uint256) {
        ...
        if (winningPosition == _YES) {
            ...
            return 0; // if market is cancelled, cannot claim
        }

        uint256 claimableAmount = winningTokenBalance * (10 ** paymentTokenDecimals) / (10 ** tokenDecimals);
    ...
    function getAllAmounts() external view returns (uint256, uint256, uint256) {
        return
            (resolverBondAmount, disputerBondAmount, escalatorBondAmount);
    }
    ...
    function _transferRewardToResolver(address _resolver) private notPaused {
        uint256 rewardBalance = rewardToken.balanceOf(address(this));
        if (rewardBalance == 0) {
            return;
        }
        rewardToken.safeTransfer(_resolver, rewardBalance);
    }
    ...
    function _settleBonds() private {
        ...
        if (escalatedDisputeAt > 0) {
            ...
            if (lastEscalation.resultWinningPosition == _CANCELED) {
                _transferRewardToResolver(marketManager.safeBoxAddress());
            } else if (lastEscalation.resultWinningPosition == lastDispute.originalOutcomeFromResolver) {
                _transferRewardToResolver(marketManager.resolverAddress(address(this)));
            } else {
                _transferRewardToResolver(lastDispute.disputorAddress);
            }
    ...
    function _isValidTransition(MarketStatus from, MarketStatus to) private view returns (bool) {
        ...
        if (from == MarketStatus.DisputeRaised) {
            return to == MarketStatus.SetByCouncil || to == MarketStatus.ResetByCouncil || to == MarketStatus.OpenForResolution;
        }
