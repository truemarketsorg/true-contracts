...
contract TruthMarketV2 is Initializable, OwnableUpgradeable, OraclePausable, ReentrancyGuardUpgradeable {
    ...
    PoolId public yesPoolId;
    PoolId public noPoolId;
    ...
    /// @param _fee The fee rate for the V4 pools
    /// @param _tickSpacing The tick spacing for the V4 pools
    /// @param _hookAddress The hook address for the V4 pools
    function initialize(
        ...
        // V4: Store hook address
        hookAddress = _hookAddress;

        // V4: Assemble pool keys (pools are created externally by TruthMarketLPManager)
        // Note: currency0 must be < currency1, so we need to sort them
        
        // YES token pool key
        (Currency currency0Yes, Currency currency1Yes) = _sortCurrencies(_yesToken, _paymentToken);
        yesPoolKey = PoolKey({
            currency0: currency0Yes,
            currency1: currency1Yes,
            fee: _fee,
        ...
        // NO token pool key
        (Currency currency0No, Currency currency1No) = _sortCurrencies(_noToken, _paymentToken);
        noPoolKey = PoolKey({
            currency0: currency0No,
            currency1: currency1No,
            fee: _fee,
            tickSpacing: _tickSpacing,
            hooks: IHooks(hookAddress)
        });
    ...
    // V4-specific getter functions
    function getPoolKeys() external view returns (PoolKey memory, PoolKey memory) {
        return (yesPoolKey, noPoolKey);
    }

    function getPoolIds() external view returns (PoolId, PoolId) {
        return (yesPoolId, noPoolId);
    }
    ...
    // V4 helper functions
    function _sortCurrencies(address token0, address token1) private pure returns (Currency, Currency) {
        if (uint160(token0) < uint160(token1)) {
            return (Currency.wrap(token0), Currency.wrap(token1));
        } else {
            return (Currency.wrap(token1), Currency.wrap(token0));
        }
    }
