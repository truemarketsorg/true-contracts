...
/// @title Launchpad Factory Community V2 Library
/// @notice Community market specific logic for Launchpad factory
library LaunchpadFactoryCommunityV2 {
    /// @notice Normalize Community market specification by applying defaults
    /// @param spec The specification to normalize (calldata)
    /// @return normalized The normalized specification (memory copy)
    function normalizeSpec(ILaunchpadFactory.CommunityTruthMarketV2Spec calldata spec)
        internal
        pure
        returns (ILaunchpadFactory.CommunityTruthMarketV2Spec memory normalized)
    {
        normalized = spec;
    ...
    /// @notice Validate Community market specification
    /// @param spec The specification to validate
    function validateSpec(ILaunchpadFactory.CommunityTruthMarketV2Spec memory spec, uint256 minimumTradingDuration)
        internal
        pure
    {
        if (spec.proposalType != ILaunchpadFactory.ProposalType.CommunityMarketV2) {
    ...
    /// @notice Compose Community market proposal
    ...
    function composeProposal(ILaunchpadFactory.CommunityTruthMarketV2Spec memory spec, address creator)
        internal
        pure
        returns (Proposal memory proposal)
    {
        ...
        proposal = LaunchpadFactoryMarketV2.createBaseProposal(
            creator, ModuleNames.DISTRIBUTOR_COMMUNITY, spec.marketParams, spec.endOfProposal
        );

        // Compose hook bindings
        // 5 hooks: outcome restrict (before), min deposit (before), min deposit (withdraw), min deposit (check), max ratio
        HookBinding[] memory hooks = new HookBinding[](5);
        ...
        // Always include minimum deposit restriction with check proposal hook
        hooks[2] = HookBinding({
